...
def _truncate(text: str, max_chars: int, marker: str) -> str:
    ...
    if len(cleaned) <= max_chars:
        return cleaned
    return cleaned[: max(0, max_chars - 20)].rstrip() + f"\n{marker}"
...
def build_thread_quote(
    ...
    *,
    ...
    """
    copy = email_copy(inbound_locale(locale))
    from_name = copy["from_name"]
    truncated = copy["message_truncated"]
    sender_label = copy["sender_label"]
    wrote = copy["wrote"]
    wrote_dated = copy["wrote_dated"]
    previous = copy["previous_messages"]
    ...
    for parsed in thread_messages:
        ...
        sender = parsed.from_address.strip().lower()
        if sender and sender in lex_addresses:
            label = f"{from_name} <{LEX_FROM_ADDRESS}>"
        else:
            label = parsed.from_address or sender_label
        date = (parsed.date_header or "").strip()
        header = (
            wrote_dated.format(date=date, label=label)
            if date
            else wrote.format(label=label)
        )
        entries.append((header, _truncate(body, max_chars_per_message, truncated)))
