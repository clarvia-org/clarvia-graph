...
from app.email.templates import RATE_LIMIT_BODY_EMERGENCY
...
def test_ten_model_eligible_requests_allowed_per_day() -> None:
...
def test_eleventh_request_is_blocked() -> None:
    clock = FakeClock(datetime(2026, 7, 25, 10, 0, tzinfo=UTC))
    store = InMemoryRateLimit()
    sender_hmac = compute_sender_hmac("user@example.com", "test-secret")
    for _ in range(10):
...
def test_rate_limit_wording_is_compassionate() -> None:
    lowered = RATE_LIMIT_BODY_EMERGENCY.casefold()
