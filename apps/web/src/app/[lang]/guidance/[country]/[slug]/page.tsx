...
import { guideBodyForLanguage } from "@/content/guide-bodies";
...
export async function generateMetadata({
  ...
  const guide = guidesForLanguage(lang).find((item) => item.slug === slug);
  return pageMetadata({
    lang,
    pathAfterLang: `guidance/${country}/${slug}`,
    title: `${guide?.title ?? tr(lang, "Guidance")} | Clarvia`,
    description: guide?.card ?? tr(lang, "Published bereavement guidance from Clarvia."),
    translated: true,
  });
}
...
export default async function GuidePage({
  ...
  const guide = guidesForLanguage(lang).find((item) => item.slug === slug);
  const body = guideBodyForLanguage(lang, slug);
  ...
  return (
    <>
      ...
      >
        <nav className="text-sm text-calm-blue-500 mb-6" aria-label={tr(lang, "Breadcrumb")}>
          <Link href={`/${lang}/guidance`} className="underline">
            {tr(lang, "Guidance")}
          </Link>
          <span aria-hidden="true"> / </span>
          <span>{guidanceCountryLabel(lang)}</span>
          <span aria-hidden="true"> / </span>
