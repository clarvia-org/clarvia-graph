...
import { type Lang, LANGUAGES, l, s1 } from "@/lib/i18n";
...
export async function generateMetadata({
  ...
  return pageMetadata({
    ...
    title: l(lang, "Updates — Clarvia", "Actualités | Clarvia", "Aktuelles | Clarvia", "Neiegkeeten | Clarvia"),
    description: l(
      lang,
...
export default async function UpdatesPage({
  ...
  const more = UPDATES.filter((update) => MORE_SERVICE_DATES.has(update.date) && !featuredDates.has(update.date));
  ...
  return (
    <>
      ...
      <main id="main-content" className="flex-grow w-full max-w-3xl mx-auto px-4 sm:px-6 py-16 relative z-10">
        <h1 className="text-4xl sm:text-5xl font-semibold tracking-tight mb-3" style={headlineStyle}>
        ...
        <section className="mb-14" aria-labelledby="latest-from-heading">
          <h2 id="latest-from-heading" className="text-2xl font-semibold text-calm-blue-800 mb-6" style={headlineStyle}>
            {s1("Latest from Clarvia")}
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {featured.map(({ slug, update }) => (
              <Link key={slug} href={`/${lang}/updates/${slug}`} className="glass-panel p-5 hover:shadow-md transition-shadow">
        ...
        <section className="mb-14" aria-labelledby="more-updates-heading">
          <h2 id="more-updates-heading" className="text-2xl font-semibold text-calm-blue-800 mb-4" style={headlineStyle}>
            {s1("More service and community updates")}
          </h2>
          <ul className="space-y-3">
            {more.map((update) => (
              <li key={`${update.date}-${update.headline.en}`} className="text-base text-calm-blue-700">
        ...
        <section aria-labelledby="project-notes-heading">
          <h2 id="project-notes-heading" className="text-2xl font-semibold text-calm-blue-800 mb-4" style={headlineStyle}>
            {s1("Project notes")}
          </h2>
